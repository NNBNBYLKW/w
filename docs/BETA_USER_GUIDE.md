...
- **A game launcher.** It tracks game status (Playing, Completed, Shelved) but does not launch games.
...
- **An AI auto-classification platform.** Suggestions are rule-based and local. No cloud AI, no LLM, no training.

## 3. Core Concepts

### Source
A folder on your computer that you want Workbench to scan. Sources are where files come from. You can have multiple sources (e.g., `G:\Downloads`, `D:\Media\Inbox`).

### Managed Library Root
A folder where Workbench can organize files. When you run an organize plan, files are moved from source folders into this managed library. Only use folders you control.
...
24. Click "Generate Rollback" (Phase 5 section).
